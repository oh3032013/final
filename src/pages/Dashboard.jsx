...
const PortfolioManager = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      try {
        const res2 = await axios.get(`${API_BASE}/api/public/projects`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('هل أنت متأكد أنك تريد حذف هذا المشروع نهائياً؟')) {
      try {
        const token = localStorage.getItem('omar_admin_token');
        const res = await axios.delete(`${API_BASE}/api/admin/projects/${id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (res.data.success) {
  ...
  const renderMedia = (project) => {
    if (project.youtubeId) {
      ...
      return (
        <div style={{ width: '60px', height: '60px', borderRadius: '8px', overflow: 'hidden', background: '#000', position: 'relative' }}>
          <video style={{ width: '100%', height: '100%', objectFit: 'cover' }} muted>
            <source src={`${API_BASE}${project.mediaUrl}`} />
          </video>
          <div style={{ position: 'absolute', bottom: '0', left: '0', right: '0', background: 'rgba(0,0,0,0.7)', fontSize: '8px', color: '#fff', textAlign: 'center' }}>
            🎬 فيديو
          </div>
      ...
      return (
        <img
          src={`${API_BASE}${project.image || project.mediaUrl}`}
          alt={project.title}
...
function Dashboard() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      // 2. جلب الفيديوهات
      const videosRes = await fetch(`${API_BASE}/api/admin/videos`, { headers });
      ...
      const settingsRes = await fetch(`${API_BASE}/api/admin/all-settings`, { headers });
      ...
      // 4. جلب الرسائل
      const messagesRes = await fetch(`${API_BASE}/api/admin/messages`, { headers });
  ...
  const handleYoutubeSync = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/admin/youtube/sync`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
  ...
  const handleVideoDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/admin/videos/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
  ...
  const handleGearSave = async () => {
    ...
    const token = getToken();
    try {
      const response = await fetch(`${API_BASE}/api/admin/gear`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleScheduleAndFAQSave = async () => {
    ...
    try {
      // حفظ الجدول
      const resSch = await fetch(`${API_BASE}/api/admin/schedule`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      ...
      // حفظ الأسئلة
      const resFaq = await fetch(`${API_BASE}/api/admin/faqs`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleSettingsSave = async (e) => {
    ...
    try {
      // حفظ الإعدادات
      const resSet = await fetch(`${API_BASE}/api/admin/settings`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      ...
      // حفظ روابط التواصل
      const resSoc = await fetch(`${API_BASE}/api/admin/socials`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleSelectMessage = async (msg) => {
    ...
    const token = getToken();
    try {
      const response = await fetch(`${API_BASE}/api/admin/messages/${msg.id}/read`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
